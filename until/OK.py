...
def Login(studentId, password):
    while True:
        S = session()
        images = S.get("http://jw1.wucc.cn:2222/CheckCode.aspx").content
        ...
        login_data = {
            '__VIEWSTATE': 'dDwyODE2NTM0OTg7Oz5vsQuFmbbIO7Cz+GqDErkUTcXRcw==',
        ...
        text = S.post("http://jw1.wucc.cn:2222/default2.aspx", data=login_data).text
        # print(text)
...
def get_score(S, username, studentId):
    new_referer = {'referer': 'http://jw1.wucc.cn:2222/xs_main.aspx?xh=' + studentId}
    S.headers.update(new_referer)

    username = username[0].replace("同学", "")
    url_history_grade = "http://jw1.wucc.cn:2222/" + (
        'xscjcx.aspx?'
        'xh={0}'
        '&xm={1}'
        '&gnmkdm=N121605'
